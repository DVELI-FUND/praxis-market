...
// GET /v1/query/votes?market=<hex>  -> panel vote status for a market
...
// GET /v1/query/slashes?address=<hex>      -> fetch a single slash record
...
// GET /v1/query/position?market=<hex>&address=<hex>  -> a single address's position
...
// GET /v1/query/unbonding?address=<hex>  -> per-resolver unbonding status
...
// GET /v1/query/dispute-context?market=<hex>&address=<hex>
...
// GET /v1/query/reward-context?pool=<resolver|builder|community|investor|protocol>&address=<hex>&epoch=<n>
// Powers the reward claim page, branched by pool type. Read-only mirror of the
// qualification checks in the corresponding DeliverMessageClaim*Reward handler.
//
// IMPORTANT: only the resolver pool is epoch-scoped (KeyForResolverEpochPool).
// builder/community/investor/protocol pools are singletons (KeyForBuilderPool
// etc., see keys_pris.go 0x22-0x25) with cooldowns tracked as a block height
// (KeyForBuilderLastClaimed / KeyForInvestorLastClaimed), not an epoch number.
// `epoch` is accepted and echoed back for those pools for URL-shape consistency
// with the resolver route, but it has no bearing on their eligibility or payout.
...
json.NewEncoder(w).Encode(result)
return
case "builder", "community", "investor", "protocol":
var (
poolKey        []byte
lastClaimedKey []byte
authorizedAddr []byte
...
switch poolType {
case "builder":
poolKey = KeyForBuilderPool()
lastClaimedKey = KeyForBuilderLastClaimed()
authorizedAddr = PRAXIS_BUILDER_ADDR
cooldownBlocks = PRIS_BUILDER_EPOCH_BLOCKS
case "community":
poolKey = KeyForCommunityPool()
authorizedAddr = PRAXIS_COMMUNITY_ADDR
case "investor":
poolKey = KeyForInvestorPool()
lastClaimedKey = KeyForInvestorLastClaimed()
authorizedAddr = PRAXIS_INVESTOR_ADDR
cooldownBlocks = PRIS_INVESTOR_VESTING_BLOCKS
case "protocol":
poolKey = KeyForProtocolPool()
authorizedAddr = PRAXIS_PROTOCOL_ADDR
}
keys := []*PluginKeyRead{{QueryId: 1, Key: poolKey}}
if lastClaimedKey != nil {
keys = append(keys, &PluginKeyRead{QueryId: 2, Key: lastClaimedKey})
}
resp, qErr := p.QueryState(0, &PluginStateReadRequest{Keys: keys})
if qErr != nil {
http.Error(w, qErr.Msg, http.StatusInternalServerError)
return
}
...
lastClaimedHeight := uint64(0)
if lastClaimedKey != nil {
if v := getEntry(2); v != nil {
lc := &LastClaimedBlock{}
Unmarshal(v, lc)
lastClaimedHeight = lc.Height
}
}
...
reason := ""
switch {
case !bytes.Equal(addr, authorizedAddr):
eligible = false
reason = "address is not the authorized " + poolType + " wallet"
case pool.Amount == 0:
eligible = false
reason = "pool is empty"
case lastClaimedKey != nil && lastClaimedHeight > 0 && height < lastClaimedHeight+cooldownBlocks:
eligible = false
...
"eligible":           eligible,
"eligible_reason":    reason,
"last_claimed_epoch": uint64(0),
"last_claimed_block": lastClaimedHeight,
"pool_type":          poolType,
}
w.Header().Set("Content-Type", "application/json")
json.NewEncoder(w).Encode(result)
return
default:
http.Error(w, "invalid pool: must be one of resolver, builder, community, investor, protocol", http.StatusBadRequest)
return
}
})
