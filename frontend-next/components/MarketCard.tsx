    ...
    >
      {imgUrl ? (
        <div className="relative h-[88px] w-full overflow-hidden">
          <img src={imgUrl} alt="" loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" onError={(e) => { e.currentTarget.parentElement!.style.display = "none"; }} />
      ...
      <div className="flex flex-1 flex-col px-4 pb-3 pt-3">
        ...
        <div className="mb-2 flex items-baseline justify-between">
          <span className={`inline-block font-display font-extrabold tracking-[-0.5px] text-up tabular-nums ${flash === "up" ? "tick-up" : flash === "down" ? "tick-down" : ""} ${featured ? "text-[30px]" : "text-[24px]"}`}>
            {pct}
        ...
        <div className="mt-auto flex items-center justify-between border-t border-line pt-2 font-mono text-[9px] text-ink-3">
          <span>Vol <b className="text-[10px] text-cyanx">{vol}</b></span>
          <span className="tabular-nums">{market.expiry ? "Ends " + fmtCountdown(Number(market.expiry), chain?.height ?? 0) : "—"}</span>
          <button className={`p-0.5 text-[13px] transition-colors ${bookmarked ? "text-amberx" : "text-ink-3 hover:text-amberx"}`} onClick={(e) => { e.preventDefault(); e.stopPropagation(); onToggleBookmark(market.marketId); }} title="Bookmark">
            {bookmarked ? "★" : "☆"}
          </button>
        </div>
      </div>
